...
var palindrome = function (string) {
  let cleaned = string.replace(/\s/g, '').toLowerCase();
  if (cleaned.length <= 1) {
    return true;
  }
...
var countValuesInObj;

// 23. Find all keys in an object (and nested objects) by a provided name and rename
// them to a provided new name while preserving the value stored at that key.
var replaceKeysInObj;
...
var alternateSign = function (array, index = 0) {
  ...
  let value = Math.abs(array[0]);
  if (index % 2 === 1) {
    value = -value;
  }
  return [value].concat(alternateSign(array.slice(1), index + 1));
};
